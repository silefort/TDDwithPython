...
repo_url = 'https://github.com/silefort/TDDwithPython'
...
def _update_settings(source_folder, site_name, suffix):
    settings_path = source_folder + suffix + '/superlists/settings.py'
    sed(settings_path, "DEBUG = True", "DEBUG = False")
    sed(settings_path,
        'ALLOWED_HOSTS =.+$',
        f'ALLOWED_HOSTS = ["{site_name}"]'
    )
    secret_key_file = source_folder + suffix + '/superlists/secret_key.py'
...
def _update_virtualenv(source_folder, suffix):
    virtualenv_folder = source_folder + '/../virtualenv'
    if not exists(virtualenv_folder + '/bin/pip'):
        run(f'python3.6 -m venv {virtualenv_folder}')
    run(f'{virtualenv_folder}/bin/pip install -r {source_folder}{suffix}/requirements.txt')

def _update_static_files(source_folder, suffix):
